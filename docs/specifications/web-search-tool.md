...
`encrypted_content` is an opaque encrypted blob. In the
real Anthropic API, this is server-encrypted content that
...
Our proxy does not generate citations since we lack the
server-side encryption infrastructure. Clients should
handle the absence of citations gracefully.
...
turns. Our proxy converts these blocks to OpenAI format on
the request path (see `_convert_web_search_result_block` in
...
| Sampling loop | Up to 10 internal iterations | Single search per tool call |
| `page_age` | Standardized human-readable | Raw from SearXNG `publishedDate` |
| `pause_turn` stop reason | When loop limit hit | Not applicable |
| Dynamic filtering (v2) | Claude writes code to filter results | Not supported |
| `user_location` | Used for geo-relevant search | Passed through but not used by SearXNG |
| `max_uses` | Enforced server-side | Not enforced |
| `allowed_domains` / `blocked_domains` | Enforced server-side | Not enforced |
